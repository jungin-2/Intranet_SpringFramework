...
.userid, .username, .userpasswd, .userpasswd1, .telno, .email, .zip, .addr1, .addr2,.org_address {
  width: 80%;
  border:none;
...
<script>
	...
	$(document).ready(function(){
		...
		$("#btn_modify").click(function(){
			...
			$("#registerForm").attr("action","/member/memberInfoModify").submit();
...
<div class="main" align="center">
  <div class="registerForm">
    ...
    <form name="registerForm" id="registerForm" method="post" enctype="multipart/form-data"> 
	    ...
	    <input type="hidden" id="org_filename" name="org_filename" value="${member.org_filename }">
	    <input type="text" id="username" name="username" class="username" value="${member.username}" placeholder="변경할 이름">
	    <input type="text" id="org_address" class="org_address" name="org_address" value="현재주소 : ${member.address}" disabled>
        ...
        <input type="text" id="zip" class="zip" name="zip" value="${member.zipcode}" disabled placeholder="검색한 주소가 입력됩니다">       
        <input type="hidden" name="zipcode">
        <input type="text" id="addr1" class="addr1" name="addr1" disabled  placeholder="검색한 주소가 입력됩니다">
        <input type="text" id="addr2" class="addr2" name="addr2" placeholder="변경할 상세주소를 입력하세요" >
        <input type="hidden" id="address" name="address">
        <input type="text" id="telno" name="telno" class="telno" value="${member.telno}" placeholder="변경할 전화번호">
        <input type="text" id="email" name="email" class="email" value="${member.email}" placeholder="변경할 이메일">
